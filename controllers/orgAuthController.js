...
const loginUser = asyncHandler(async (req, res, next) => {
	...
	try {
		...
		const match = await bcryptjs.compare(
			password,
			foundAdminUser ? foundAdminUser.adminPassword : foundEmpUser.password
		);
		...
		if (foundAdminUser && foundAdminUser.mfaEnabled) {
			...
			return res.status(200).json({
				success: true,
				data: {
					mfaEnabled: foundAdminUser
						? foundAdminUser.mfaEnabled
						: foundEmpUser.mfaEnabled,
					secret: foundAdminUser
						? foundAdminUser.mfaSecret
						: foundEmpUser.mfaSecret,
				},
			});
		} else {
			...
			const customURL = `otpauth://totp/${encodeURIComponent(
				...
				foundAdminUser ? foundAdminUser.adminFirstName : foundEmpUser.name
			)}?secret=${secret.base32}&issuer=${encodeURIComponent("Invento")}`;
...
const verifyUserMFA = asyncHandler(async (req, res, next) => {
	...
	try {
		...
		if (isValid) {
			...
			const refreshToken = jwt.sign(
				{ email: isAdmin ? foundUser.adminEmail : foundUser.email },
				process.env.REFRESH_TOKEN_SECRET,
				{ algorithm: "HS256", expiresIn: "1d" }
			);
